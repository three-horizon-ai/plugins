...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [azureDevboxPlugin],
    });

    await request(server).get('/api/azure-devbox/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/azure-devbox/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/azure-devbox/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/azure-devbox/todos/${createdTodoItem.id}`)
